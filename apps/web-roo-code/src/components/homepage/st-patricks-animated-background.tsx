...
export function StPatricksAnimatedBackground() {
	...
	useEffect(() => {
		...
		// initialize gradient points for lighting effects (GREEN THEME)
		let gradientPoints = [
			{
				x: canvas.width * 0.2,
				y: canvas.height * 0.3,
			...
			},
			{
				...
				color: "rgba(22, 163, 74, 0.1)", // green-600
			},
		]
		...
		function drawGrid() {
			...
			ctx.strokeStyle = `rgba(34, 197, 94, ${gridOpacity})` // Green grid lines
			ctx.lineWidth = 0.5
		...
		class Particle {
			...
			constructor() {
				...
				this.color = "rgba(34, 197, 94, " // Green particles
			...
			for (let a = 0; a < particles.length; a++) {
				for (let b = a; b < particles.length; b++) {
					...
					if (distance < maxDistance) {
						...
						ctx.strokeStyle = `rgba(34, 197, 94, ${opacity})` // Green connection lines
						ctx.lineWidth = 0.5
						ctx.beginPath()
